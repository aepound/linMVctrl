...
% Anonymous wrapper around fread:
frd = @(x,y,z) fread(x,y,z);
% ###############################################
% ##                                           ##
% ##    open the file                          ##
% ##                                           ##
...
NrecordFirstParameterblock=frd(fid,1,'int8');     % Reading record number of parameter section
key=frd(fid,1,'int8');                           % key = 80;
...
proctype=frd(fid,1,'int8')-83;                       % proctype: 1(INTEL-PC); 2(DEC-VAX); 3(MIPS-SUN/SGI)

if proctype==2,
    fclose(fid);
    fid=fopen(FullFileName,'r','l'); % DEC VAX D floating point and VAX ordering
    
    % Change the fread wrapper to use the VAXD specs...
    frd = @(x,y,z) freadVAXD(x,y,z);
end
...
Nmarkers=frd(fid,1,'int16');			        %number of markers
NanalogSamplesPerVideoFrame=frd(fid,1,'int16');			%number of analog channels x #analog frames per video frame
StartFrame=frd(fid,1,'int16');		        %# of first video frame

EndFrame=frd(fid,1,'int16');			        %# of last video frame

MaxInterpolationGap=frd(fid,1,'int16');		%maximum interpolation gap allowed (in frame)

Scale=frd(fid,1,'float32');			        %floating-point scale factor to convert 3D-integers to ref system units

NrecordDataBlock=frd(fid,1,'int16');			%starting record number for 3D point and analog data

NanalogFramesPerVideoFrame=frd(fid,1,'int16');
...
VideoFrameRate=frd(fid,1,'float32');
...
fseek(fid,298,'bof');
EventIndicator=frd(fid,1,'int16');	
if EventIndicator==12345,
    Nevents=frd(fid,1,'int16');	
    fseek(fid,2,'cof'); % skip one position/2 bytes
    if Nevents>0,
        for i=1:Nevents,
            Event(i).time=frd(fid,1,'float');
        end
        fseek(fid,188*2,'bof');
        for i=1:Nevents,
            Event(i).value=frd(fid,1,'int8');
        end
         fseek(fid,198*2,'bof');
        for i=1:Nevents,
            Event(i).name=cellstr(char(frd(fid,4,'char')'));
        end
    end
end
...
dat1=frd(fid,1,'int8'); 
key2=frd(fid,1,'int8');                   % key = 80;
NparameterRecords=frd(fid,1,'int8');
proctype=frd(fid,1,'int8')-83;            % proctype: 1(INTEL-PC); 2(DEC-VAX); 3(MIPS-SUN/SGI)


Ncharacters=frd(fid,1,'int8');   			% characters in group/parameter name
GroupNumber=frd(fid,1,'int8');				% id number -ve=group / +ve=parameter
...
while Ncharacters > 0 % The end of the parameter record is indicated by <0 characters for group/parameter name
    ...
    if GroupNumber<0 % Group data
        ...
        GroupName=frd(fid,[1,Ncharacters],'char');			
        ParameterGroup(GroupNumber).name=cellstr(char(GroupName));	%group name
        offset=frd(fid,1,'int16');							%offset in bytes
        deschars=frd(fid,1,'int8');							%description characters
        GroupDescription=frd(fid,[1,deschars],'char');
        ...
        ParameterName=frd(fid,[1,Ncharacters],'char');				% name of parameter
        ...
        if type==-1 %datatype=='char'  
            ...
            if dimnum==2 & datalength>0 %& parameter(idnumber,index,2).dim>0            
                for j=1:dimension(2)
                    data=frd(fid,[1,wordlength],'char');	%character word data record for 2-D array
                    ParameterGroup(GroupNumber).Parameter(ParameterNumber).data(j)=cellstr(char(data));
                end
                
            elseif dimnum==1 & datalength>0
                data=frd(fid,[1,wordlength],'char');		%numerical data record of 1-D array
        ...
        elseif type==4 & datalength>0
            ...
            Nparameters=datalength/abs(type);
            data=frd(fid,Nparameters,'float');
            if dimnum>1
                ParameterGroup(GroupNumber).Parameter(ParameterNumber).data=reshape(data,dimension);
            else
                ParameterGroup(GroupNumber).Parameter(ParameterNumber).data=data;
        ...
        deschars=frd(fid,1,'int8');							%description characters
        if deschars>0
            description=frd(fid,[1,deschars],'char');
    ...
    Ncharacters=frd(fid,1,'int8');   			% characters in next group/parameter name
    GroupNumber=frd(fid,1,'int8');				% id number -ve=group / +ve=parameter
end
...
if Scale < 0
    for i=1:NvideoFrames
        for j=1:Nmarkers
            Markers(i,j,1:3)=frd(fid,3,'float32')'; 
            a=fix(frd(fid,1,'float32'));  
            highbyte=fix(a/256);
        ...
        for j=1:NanalogFramesPerVideoFrame,
            AnalogSignals(j+NanalogFramesPerVideoFrame*(i-1),1:NanalogChannels)=...
                frd(fid,NanalogChannels,'int16')';
        end
    end
else
    for i=1:NvideoFrames
        for j=1:Nmarkers
            Markers(i,j,1:3)=frd(fid,3,'int16')'.*Scale;
            ResidualError(i,j)=frd(fid,1,'int8');
            CameraInfo(i,j)=frd(fid,1,'int8');
        end
        waitbar(i/NvideoFrames)
        for j=1:NanalogFramesPerVideoFrame,
            AnalogSignals(j+NanalogFramesPerVideoFrame*(i-1),1:NanalogChannels)=...
