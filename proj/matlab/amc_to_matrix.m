...
% March 2003
% Databases Course
function [D, S] = amc_to_matrix(fname)
...
locations = cumsum([1 dims(1:end-1)]);
...
fnms = fieldnames(S);
T = struct;
for iter = 1:length(fnms)
    szm = size(S(1).(fnms{iter}));
    M = [S.(fnms{iter})];
    M = reshape(M,szm(2),[])';
    T(1).(fnms{iter}) = M;
end

assert(all(structfun(@(x)size(x,1),T) == frame-1))
S = T;
